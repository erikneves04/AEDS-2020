...
public class Lista {
    ...
    public void Insere_dado(int Dado){
        ...
        if(UltimoEncontrado == true){
            Novo_dado.InsereDadosItemLista(this.primeiro,Ultimo_dado,Dado);
            Ultimo_dado.SetProximo(Novo_dado);
        }else{
            Novo_dado.InsereDadosItemLista(Novo_dado,Novo_dado,Dado);
        }
    ...
    public boolean Remove_dado(int Dado){  
        ...
        if(InformacaoEncontrada == true){
            ...
            if(this.Numero_de_itens == 0){
                this.primeiro = null;
            }else if(Dado_anterior == null){
                this.primeiro = Dados_lista.GetProximo();
                this.primeiro.SetAnterior(Ultimo_dado);
                if(UltimoEncontrado == true){
